...
// MultiUpdateRequestBody is the type of the "user" service "multi_update"
...
// StoredBottleResponseTinyCollection is the type of the "user" service "list"
...
// ShowResponseBodyTiny is the type of the "user" service "show" endpoint HTTP
// response body.
...
// ShowNotFoundResponseBody is the type of the "user" service "show" endpoint
...
func NewStoredBottleResponseTinyCollection(res userviews.StoredBottleCollectionView) StoredBottleResponseTinyCollection {
	...
	for i, val := range res {
		...
		if val.Winery != nil {
			body[i].Winery = marshalUserviewsWineryViewToWineryResponseTiny(val.Winery)
		}
	}
	return body
}
...
func NewShowResponseBody(res *userviews.StoredBottleView) *ShowResponseBody {
	...
	if res.Winery != nil {
		body.Winery = marshalUserviewsWineryViewToWineryResponseBodyTiny(res.Winery)
	}
	if res.Composition != nil {
		body.Composition = make([]*ComponentResponseBody, len(res.Composition))
		for i, val := range res.Composition {
			body.Composition[i] = marshalUserviewsComponentViewToComponentResponseBody(val)
		}
	}
	return body
}
...
// "show" endpoint of the "user" service.
func NewShowResponseBodyTiny(res *userviews.StoredBottleView) *ShowResponseBodyTiny {
...
func NewShowNotFoundResponseBody(res *user.NotFound) *ShowNotFoundResponseBody {
...
func NewShowPayload(id string, view *string) *user.ShowPayload {
	return &user.ShowPayload{
		ID:   id,
		View: view,
	}
}

// NewAddBottle builds a user service add endpoint payload.
func NewAddBottle(body *AddRequestBody) *user.Bottle {
	v := &user.Bottle{
		Name:        *body.Name,
	...
	v.Winery = unmarshalWineryRequestBodyToUserWinery(body.Winery)
	if body.Composition != nil {
		v.Composition = make([]*user.Component, len(body.Composition))
		for i, val := range body.Composition {
			v.Composition[i] = unmarshalComponentRequestBodyToUserComponent(val)
		}
	}
	return v
}

// NewRemovePayload builds a user service remove endpoint payload.
func NewRemovePayload(id string) *user.RemovePayload {
	return &user.RemovePayload{
		ID: id,
	}
}

// NewMultiAddBottle builds a user service multi_add endpoint payload.
func NewMultiAddBottle(body []*BottleRequestBody) []*user.Bottle {
	...
	for i, val := range body {
		...
		v[i].Winery = unmarshalWineryRequestBodyToUserWinery(val.Winery)
		if val.Composition != nil {
			v[i].Composition = make([]*user.Component, len(val.Composition))
			for j, val := range val.Composition {
				v[i].Composition[j] = unmarshalComponentRequestBodyToUserComponent(val)
			}
		}
	}
	return v
...
// NewMultiUpdatePayload builds a user service multi_update endpoint payload.
func NewMultiUpdatePayload(body *MultiUpdateRequestBody, ids []string) *user.MultiUpdatePayload {
	v := &user.MultiUpdatePayload{}
	v.Bottles = make([]*user.Bottle, len(body.Bottles))
	for i, val := range body.Bottles {
		v.Bottles[i] = unmarshalBottleRequestBodyToUserBottle(val)
	}
	v.Ids = ids
	return v
}
