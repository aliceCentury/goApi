...
// UserMultiAddDecoderFunc is the type to decode multipart request for the
// "user" service "multi_add" endpoint.
type UserMultiAddDecoderFunc func(*multipart.Reader, *[]*user.Bottle) error

// UserMultiUpdateDecoderFunc is the type to decode multipart request for the
// "user" service "multi_update" endpoint.
type UserMultiUpdateDecoderFunc func(*multipart.Reader, **user.MultiUpdatePayload) error
...
func New(
	...
	userMultiAddDecoderFn UserMultiAddDecoderFunc,
	userMultiUpdateDecoderFn UserMultiUpdateDecoderFunc,
) *Server {
	return &Server{
		Mounts: []*MountPoint{
			{"List", "GET", "/user"},
			...
			{"Rate", "POST", "/user/rate"},
			{"MultiAdd", "POST", "/user/multi_add"},
			{"MultiUpdate", "PUT", "/user/multi_update"},
		},
		...
		MultiAdd:    NewMultiAddHandler(e.MultiAdd, mux, NewUserMultiAddDecoder(mux, userMultiAddDecoderFn), enc, eh),
		MultiUpdate: NewMultiUpdateHandler(e.MultiUpdate, mux, NewUserMultiUpdateDecoder(mux, userMultiUpdateDecoderFn), enc, eh),
	}
}
...
// MountRemoveHandler configures the mux to serve the "user" service "remove"
// endpoint.
...
// MountRateHandler configures the mux to serve the "user" service "rate"
// endpoint.
...
// MountMultiAddHandler configures the mux to serve the "user" service
// "multi_add" endpoint.
...
// MountMultiUpdateHandler configures the mux to serve the "user" service
...
func MountMultiUpdateHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("PUT", "/user/multi_update", f)
}

// NewMultiUpdateHandler creates a HTTP handler which loads the HTTP request
// and calls the "user" service "multi_update" endpoint.
