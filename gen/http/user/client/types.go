...
func NewAddRequestBody(p *user.Bottle) *AddRequestBody {
	body := &AddRequestBody{
		Name:        p.Name,
		Vintage:     p.Vintage,
	...
	if p.Winery != nil {
		body.Winery = marshalUserWineryToWineryRequestBody(p.Winery)
	}
	if p.Composition != nil {
		body.Composition = make([]*ComponentRequestBody, len(p.Composition))
		for i, val := range p.Composition {
			body.Composition[i] = marshalUserComponentToComponentRequestBody(val)
		}
	}
	return body
}
...
func NewBottleRequestBody(p []*user.Bottle) []*BottleRequestBody {
	...
	for i, val := range p {
		...
		if val.Winery != nil {
			body[i].Winery = marshalUserWineryToWineryRequestBody(val.Winery)
		}
		if val.Composition != nil {
			body[i].Composition = make([]*ComponentRequestBody, len(val.Composition))
			for j, val := range val.Composition {
				body[i].Composition[j] = marshalUserComponentToComponentRequestBody(val)
			}
		}
	}
	return body
...
// the "multi_update" endpoint of the "user" service.
func NewMultiUpdateRequestBody(p *user.MultiUpdatePayload) *MultiUpdateRequestBody {
	body := &MultiUpdateRequestBody{}
	if p.Bottles != nil {
		body.Bottles = make([]*BottleRequestBody, len(p.Bottles))
		for i, val := range p.Bottles {
			body.Bottles[i] = marshalUserBottleToBottleRequestBody(val)
		}
	}
	return body
}

// NewListStoredBottleCollectionOK builds a "user" service "list" endpoint
// result from a HTTP "OK" response.
func NewListStoredBottleCollectionOK(body ListResponseBody) userviews.StoredBottleCollectionView {
	v := make([]*userviews.StoredBottleView, len(body))
	for i, val := range body {
		v[i] = &userviews.StoredBottleView{
			ID:          val.ID,
			Name:        val.Name,
			Vintage:     val.Vintage,
			Description: val.Description,
		...
		v[i].Winery = unmarshalWineryResponseToUserviewsWineryView(val.Winery)
		if val.Composition != nil {
			v[i].Composition = make([]*userviews.ComponentView, len(val.Composition))
			for j, val := range val.Composition {
				v[i].Composition[j] = unmarshalComponentResponseToUserviewsComponentView(val)
			}
		}
	}
	return v
...
// NewShowStoredBottleOK builds a "user" service "show" endpoint result from a
// HTTP "OK" response.
func NewShowStoredBottleOK(body *ShowResponseBody) *userviews.StoredBottleView {
	v := &userviews.StoredBottleView{
		ID:          body.ID,
		Name:        body.Name,
	...
	v.Winery = unmarshalWineryResponseBodyToUserviewsWineryView(body.Winery)
	if body.Composition != nil {
		v.Composition = make([]*userviews.ComponentView, len(body.Composition))
		for i, val := range body.Composition {
			v.Composition[i] = unmarshalComponentResponseBodyToUserviewsComponentView(val)
		}
	}
	return v
}

// NewShowNotFound builds a user service show endpoint not_found error.
func NewShowNotFound(body *ShowNotFoundResponseBody) *user.NotFound {
	v := &user.NotFound{
